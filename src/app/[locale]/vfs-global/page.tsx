...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const data = VFS_PAGES['vfs-global'];
    return {
        title: data.metaTitle,
        description: data.metaDescription,
        alternates: {
            canonical: `https://indonesianvisas.com/${locale}/vfs-global`,
        }
    };
}

export default async function VFSGlobalPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const data = VFS_PAGES['vfs-global'];
